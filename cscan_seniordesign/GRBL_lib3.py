...
@author: Mike
Updated to support 3 axis controls
"""
...
''' GRBL SCALE FACTOR: Because GRBL is retarded and won't properly move motors 
with low mm/in. It is developed by taking the (desired movement(mm))/(actual(mm))'''
GRBL_Scale = 0.01124859392575928
###############################################################################
...
def writeStepsPerMM(s, x_step, y_step=-1, z_step=-1):
    ''' Too lazy to type into GRBL new steps per mm for all 3 axis. Optional
    args will use x_step input as the same for all '''
    
    if (y_step == -1 or z_step == -1):
        # some settings were left out, assign them
        y_step = x_step
        z_step = x_step
        print('made it into the else')
        
    x_step = '$100=' + str(x_step) + '\n'
    y_step = '$101=' + str(y_step) + '\n'
    z_step = '$102=' + str(z_step) + '\n'
    settings_list = [x_step, y_step, z_step]
    for setting in settings_list:
        Console.timeStamp('Sending: ', setting)
        s.write(setting.encode('utf-8'))
        printAckErrorCheck(s)
        print('\n')
    getSettings(s)
    Console.timeStamp('Check the above settings printout to verify settings' +
                      'have been set')
...
def feedrateMove(s, x, y, z):
    ...
    s.write(str.encode('G1 X' + str(x) + ' Y' + str(y) + ' Z' + str(z) + '\r\n'))
    ...
    while 1:
        ...
        status = s_response.replace('<', ' ').replace('|', ' ').split()
        '''NEW GRBL requires a '|' instead of a ','. Change these. '''
...
def fastMove(s, x, y, z):
    ...
    #TODO: why just \n instead of \r\n like for feedrate move???
    s.write(str.encode('G0 X' + str(x) + ' Y' + str(y) + ' Z' + str(z) + '\n'))
    print('Tx Status: ' +  s.readline().decode('utf-8'))
...
def defineFeedrate(s, feedrate):
    ''' Sends a feedrate to GRBL since after V0.9, a default feedrate required
    for use of the G1 command was removed because it is 'illegal'. '''
    
    Console.timeStamp('Sending F0 (feedrate definition)....')
    
    # 
    # Send the movement command
    s.write(str.encode('F0 ' + str(feedrate) + '\r\n'))
    print('Tx Status: ' +  s.readline().decode('utf-8'))
...
def testFeedrateMove(s, x, y): 
    ...
    # Added in Grbl_lib3: (just in case)
    x = x*GRBL_Scale
    y = y*GRBL_Scale
    z = z*GRBL_Scale
