...
Cscan supporting functions (generating points etc.), and some test functions.
...
################################# Test Modes ##################################     
def movTestMode(s):
    ''' Test Gcode sending functions with hardcoded values '''
    
    # Do something
    # Note absolute coordinate system
    #Grbl.testFeedrateMove(s, 10, 15)  
    Grbl.feedrateMove(s, 20, 0, 0)
    Grbl.pause(s, 5)    
    Grbl.fastMove(s, 0, 0, 0)
    
def daqTestMode(Instrument, filename):
    ''' Test some VISA commands. '''
...
################################# Other Modes #################################    
   ## TODO: update for 3d movement
def genPoints():
    ''' This is the giant function that comes up with the path the machine
    should take to cover a 2D space. This method is still used by Cscan. '''
