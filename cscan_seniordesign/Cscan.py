...
# TODO: Robustness: if scan 50 mm but with 20 mm step, it goes to 60. problem.
###############################################################################
################################### 2D Scan ###################################
###############################################################################
def cScan(CNC, DaQ_instrument, CNC_dwell, DaQ_dwell):
    Console.timeStamp('2D scan time!')
    
    # Query user about size of object under test and how to approach
    ...
    # The genPoints function in Cscan lib generates points based on user input        
    ...
    print('\n###############################################################' +
            '################\n')
    # ...This looks way cooler when the program is run..
    ...
    # V3 Improvement:
    # See which string has more points (this should do this now even tho V2)
    if len(x_points) >= len(y_points):
        # Default traversal copied and pasted from old version
        ...
        # Alternative traversal modifed from V2
        oddloop = 0
        for j in range(len(x_points)):
            x_points[j]  = '{:1.1f}'.format(x_points[j])
    
            if oddloop == 0:
                posgen_range = range(len(y_points))
                oddloop = 1
            elif oddloop == 1:
                posgen_range = range(len(y_points)-1, -1, -1)
                oddloop = 0
            for i in posgen_range:
                y_points[i] = '{:1.1f}'.format(y_points[i])
                filename = prefix + '[' + str(x_points[j]) + ',' + str(y_points[i]) + ']' + '.S' + extension_order + 'P'
                filenames.append(filename)
                positions.append([x_points[j],y_points[i]])
                #print(filename)
        

    # confirm with user before moving machine
    # TODO: add a way to go back without aborting and have to type all that
    # crap again.
    ...
    ''' Use the GRBL set settings to turn off delayed auto-off on the holding
    torque. 255 = always on. You cannot (should not) move gantry manually 
    when motors are applying holding torque. '''
    # Get old setting, while setting new setting
    ...
    if old_setting == float(255):
        ...
        old_setting = 50 # This way the holding torque is shut off at the end
                            # when the value is restored
    ...
    ''' uncomment these to shut off holding torque (save power/wear) at end. It
    was disabled so that the machine does not move at all once finished for
    careful realignment of a different sample. '''
    #time.sleep(10)   
...
def debugMode(CNC_dwell, DaQ_dwell):
    ...
    # V3 Improvement:
    # See which string has more points
    if len(x_points) >= len(y_points):
        # Default traversal copied and pasted from old version
        # Not sure why had such issues here. Is same thing as below, but with index
        oddloop = 0
    ...
    ## INSERT SETTINGS CHANGE IF REAL CNC HERE ##
